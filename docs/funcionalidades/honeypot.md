...
Sabe aquele spammer que entra no servidor e sai disparando mensagem em todo canal que vê pela frente? O Honeypot é a armadilha perfeita pra ele. Você cria um canal onde ninguém de verdade deveria escrever — e quem escrever ali é punido na hora, com a mensagem apagada e um alerta na mão da sua equipe.
...
A ideia é simples: você deixa um canal **visível, mas onde nenhum membro legítimo deveria postar** — tipo um canal chamado "nao-escreva-aqui" lá no topo da lista. Membros reais leem o aviso e passam direto. Bots de spam e contas-laranja, não: eles escrevem em tudo que veem e caem direto na isca.

Quando alguém morde a isca, o Delfus age em sequência, em questão de segundos:

1. **Apaga a mensagem** na hora (se tiver permissão pra isso).
...
4. **Para por aí.** A mensagem não passa por mais nada (triggers, filtros, contadores) — o Honeypot tem prioridade.

!!! example "Exemplo"
    Uma conta nova entra no servidor e dispara um print de golpe em vários canais, incluindo o seu "🚫-nao-escreva-aqui". O Delfus apaga a mensagem, silencia o autor por 10 minutos e manda um aviso pro canal da staff. Seus membros nem chegam a ver o golpe.
...
!!! note "Detalhes que valem saber"
    ...
    - A punição só funciona se o **cargo do bot estiver acima do membro** na hierarquia. Se não estiver, a mensagem ainda é apagada e o alerta ainda chega — só a punição em si pode falhar.
    - Sua configuração fica **salva no banco** e volta sozinha quando o bot reinicia. Configurou uma vez, funciona pra sempre (até você desligar).
...
| `/honeypot mensagem` | Personaliza o alerta da equipe — como **texto** simples ou como **embed** (com título e cor). |
...
!!! note "Sobre os comandos"
    ...
    - Os campos de canal têm **autocompletar** pra facilitar.

## Configuração

Dá pra configurar de dois jeitos — e os dois mexem na mesma config, então pode misturar à vontade.
...
1. Rode `/honeypot ativar` com o canal isca e o canal de alertas. Pronto — já fica ativo silenciando por 10 minutos.
...
!!! example "Banir contas maliciosas sem moderador online"
    Depois de ativar, rode `/honeypot acao` e escolha **Banir**. A partir daí, quem cai na armadilha é banido na hora — não precisa de ninguém da staff acordado às 3 da manhã.

!!! example "Alerta bonito pra equipe"
    Rode `/honeypot mensagem` com tipo **embed**, título "⚠️ Honeypot acionado" e descrição "O usuário {user} caiu na isca {channel} e foi punido." O aviso chega formatadinho e já com as menções certas.
...
!!! tip "Dica"
    Coloque o canal isca bem no topo da lista, com um nome claro de "não escreva aqui". Spammers escrevem nos primeiros canais que veem; membros reais leem o aviso e seguem em frente. Combine com a ação **Banir** e deixe as contas maliciosas saírem sozinhas, sem depender de moderador online.
